...
void receiveEvent(int howMany) {
  if (Wire.available()) {
    ...
    if (received == 'B') { // Se for 'B', revela a base
      Serial.write('B');
      for (i = 0; i < N; i++) {
        Serial.write(basisAlice[i]); // Envia o valor dos bits medidos para o Processing
      }
    }
    ...
    /* Cada bit/base é enviado individualmente por isso precisa do contador */
      ...
      if (countAlice >= 16) {
        countAlice = 0;  // Quando a contagem passar de 16, zera e recomeça
      }
...
void loop() {
  ...
  if (countAlice == 16 && countBob == 8 && flag == 1) {
    Serial.write('S'); // Envia S para dizer que vai começar a enviar os bits
    for (i = 0; i < N; i++) {
